...
import { ProviderManager } from '../../src/rate-provider'
import { UPLOAD_FOLDER } from '../../src/rate-updater'
import UploadJob from '../../src/rate-updater/upload.model'
...
describe('Upload service', function () {
  ...
  it('should create rate-updater folder', () => {
  ...
  describe('Upload API', () => {
    ...
    it('should rate-updater file', async () => {
    ...
    it('should rate-updater file and unpin when receive pinned message', async () => {
      const response = await upload(testApp.providerAddress, 'testAccount', testApp.peerId?.id as string, ['./test/integration/files/testFile.txt'])
      expect(response.message).to.be.eql('Files uploaded')
      expect(response.fileHash).to.be.not.null()
      const jobs = await UploadJob.findAll()
